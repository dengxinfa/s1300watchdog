...
#define   watchdog_of_b1300_s    400 //时间单位 30*50 每计数50次为一秒
...
//	
	if(dog_count > dog_time){
		if(dog_flags == 1){
			...
			dog_flags = 1;  //关闭复位主控功能
		}
		dog_count = 0;  //清零计数器，同时防止dog_count溢出
	} 
	
	if(dog_count > 100 && int_number !=0){     //在1秒时，中断次数（中断信号频率需要大于0.5Hz）；3~7：看门狗开始计时；>10看门狗停止计时
